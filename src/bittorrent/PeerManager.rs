...
impl PeerManager {
            ...
            match stream.read(&mut buf) {
                ...
                Ok(n) => {
                    match peer.parse_message(&buf) {
                        ...
                        Err(e) => {
                            error!("Error while parsing incoming message from peer {:?}: {}",
                             peer.peer_id(), e);
                        }
                    }
                },
    ...
    fn process_queue(iter : &mut mpsc::TryIter<PeerUpdate>, peer_priority : &mut PriorityQueue<[u8 ; 20], PeerPriority>,
        bitfield : &mut BitVec, peer_id : &[u8; 20]) -> bool {
        match iter.next() {
            Some(msg) => {
    ...
    fn manage_choking(recv : mpsc::Receiver<NewPeerMsg>) {
        ...
        loop {
            ...
            peers.retain(|peer_id, comm| {
                ...
                PeerManager::process_queue(&mut comm.try_iter(), &mut peer_priority, bitfield, &peer_id)
            });
